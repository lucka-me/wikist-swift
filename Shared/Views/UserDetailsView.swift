...
struct UserDetailsView: View {
    ...
    var body: some View {
        ...
        .onReceive(contributions.publisher.count()) { count in
            guard
                isStatisticsReady,
                statistics.contributionsCount != count else {
                return
            }
            if let statisticsTask {
                statisticsTask.cancel()
            }
            statisticsTask = .init {
                guard let statistics = await makeStatistics() else { return }
                await MainActor.run {
                    withAnimation {
                        self.statistics = statistics
                    }
    ...
    @MainActor
    private func makeStatistics() async -> Statistics? {
